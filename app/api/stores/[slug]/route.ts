...
async function fetchRobloxProducts(groupId: string, retries: number = 3): Promise<any> {
    const response = await fetch(`https://catalog.roblox.com/v2/search/items/details?Category=3&CreatorType=Group&CreatorTargetId=${groupId}`);
    const data = await response.json();

    // Check for rate limiting
    if (data.errors && data.errors.some((error: any) => error.message === 'Too many requests')) {
        if (retries > 0) {
...
export async function GET(request: NextRequest, { params }: { params: { slug: string } }) {
    ...
    try {
        ...
        return NextResponse.json({ store, products: productsData.data || [] }, { status: 200 });
    } catch (error) {
        console.error("Error fetching store and products:", error);
        return NextResponse.json({ error: "Failed to fetch store and products" }, { status: 500 });
    }
} 