  ...
  const handleAddStore = async () => {
    ...
    try {
      ...
      const checkResponse = await fetch(
        `/api/stores/check?userId=${user?.id}&groupId=${groupData.id}`,
        {
          method: "GET",
        },
      );
      ...
      router.push(
        `/stores/new/setup?userId=${user?.id}&groupId=${groupData.id}&groupName=${groupData.displayName}`,
      ...
      {user && (
        <div className="text-center p-8 border border-zinc-900 shadow-lg">
          ...
          <p className="mb-8 text-zinc-400">
            enter the id of the group you want to list on shopblox:
          </p>
          <form onSubmit={handleSubmit}>
            <input
              ...
              className="w-full p-2 mb-4 border border-zinc-900 rounded-none text-zinc-300 bg-zinc-900"
            />
            <button
              type="submit"
              className="w-full p-2 bg-zinc-900 text-zinc-300 border border-zinc-800 shadow-md hover:bg-zinc-800 transition-colors"
            >
              Submit
            </button>
          </form>
          ...
          {groupData && isUserOwner() && (
            <div className="space-y-4">
              <h1 className="text-2xl font-bold text-zinc-300 mt-4">
                Is this your group?
              </h1>
              <GroupCard
                groupData={groupData}
              ...
              <div className="flex justify-between w-full">
                ...
                <Button className="w-full bg-zinc-900 text-zinc-300 border border-zinc-800 hover:bg-zinc-800 rounded-none">
                  No
                </Button>
              </div>
            </div>
              ...
              <Card className="text-center p-8 border border-zinc-900 shadow-lg bg-zinc-900 border-zinc-800 mt-4 rounded-none">
                <CardHeader>
                  <CardTitle className="text-zinc-300 font-normal">
                    ...
                    <p className="p-4 text-zinc-300 font-normal italic">
